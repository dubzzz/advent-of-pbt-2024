...
export default function nextBarcode(barcode) {
  ...
  const unitPerNumerical =
    typeof unitPerNumericalOverride !== "undefined"
  ...
  for (let i = barcode.length; i > 0; i -= unitPerNumerical) {
    const unitsForNumerical = barcode.slice(
      Math.max(0, i - unitPerNumerical),
      i
    );
    let numerical = 0;
  ...
  if (nextNumericalVersion[nextNumericalVersion.length - 1] === 0) {
    nextNumericalVersion = nextNumericalVersion.slice(
      0,
      nextNumericalVersion.length - 1
    );
  }
  ...
  for (
    ...
    numericalIndex !== nextNumericalVersion.length;
    ++numericalIndex
  ) {
